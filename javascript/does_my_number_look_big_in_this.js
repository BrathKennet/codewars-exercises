...
function narcissistic(value) {
  // Inicialización de la variable "total"
  let total = 0;

  for (let i = 0; i < value.toString().length; i++) {
    // Calcula la i-ésima potencia del dígito y la agrega al total
    total += Math.pow(
      parseInt(value.toString()[i]),
      parseInt(value.toString().length)
    );
  }

  // Compara el valor original con el total calculado para determinar si es narcisista.
  return value === total;
}